...
allowed_times = {"8:30 AM", "9:45 AM", "11:00 AM"}
...
# --- Field Coordinates (1000x1500 image assumed for Field 3) ---
field_positions = {
    "Field 1":   { "x": 40.0, "y": 15.3, "width": 16.3, "height": 15.0 },
    "Field 1A":  { "x": 40.0, "y": 25.2, "width": 16.3, "height": 15.0 },
    "Field 1B":  { "x": 40.0, "y": 15.3, "width": 16.3, "height": 15.0 },
    "Field 2":   { "x": 60.0, "y": 15.3, "width": 16.0, "height": 15.0 },
    "Field 2A":  { "x": 60.0, "y": 25.2, "width": 16.0, "height": 15.0 },
    "Field 2B":  { "x": 60, "y": 15.3, "width": 16.0, "height": 15.0 },
    "Field 3":   { "x": 15, "y": 70, "width": 16.2, "height": 15, "rotate": 7.5 },
    "Field 4":   { "x": 37.5, "y": 63, "width": 25, "height": 9.3, "rotate": 4.5 },
    "Field 4A":  { "x": 37.5, "y": 63, "width": 20, "height": 9.3, "rotate": 4.5 },
    "Field 4B":  { "x": 55.5, "y": 64, "width": 20, "height": 9.3, "rotate": 4.7 },
}
...
# --- Extract Matchups for Next Saturday ---
matchups = []
...
with open(output_html, "w", encoding="utf-8") as f:
    ...
    .team-left, .team-right {
        ...
        font-size: clamp(0.5em, 1.2vw, 0.85em);
        max-width: 100%;
    }
    .diagonal-text {
    ...
    for block in sorted(games_by_block.keys(), key=time_sort_key):
        ...
        for matchup in games_by_block[block]:
            ...
            if field in side_by_side_fields:
                ...
                f.write(f"<div class='team-left' style='flex:1; background-color:{color_map.get(matchup['color1'], '#ccc')}'>{matchup['team1']}</div>\n")
                f.write(f"<div class='team-right' style='flex:1; background-color:{color_map.get(matchup['color2'], '#ccc')}'>{matchup['team2']}</div>\n")
                f.write("</div>\n")
            else:
                block_style = "padding:10px 2px;" if enlarged else "padding:6px 2px;"
                f.write(f"<div class='team-left' style='{block_style} background-color:{color_map.get(matchup['color1'], '#ccc')}'>{matchup['team1']}</div>\n")
                f.write(f"<div class='team-right' style='{block_style} background-color:{color_map.get(matchup['color2'], '#ccc')}'>{matchup['team2']}</div>\n")
